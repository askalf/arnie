...
- "event_log" reads recent system events. Windows: Get-WinEvent against System/Application/Security. Linux: journalctl. Filter by level (error/warning/info/all), source (Windows log name), since_minutes, and max_entries. Use this for "what's wrong with the system" / "any recent errors" instead of digging through Event Viewer.
- "registry_read" reads Windows registry keys/values (Windows-only). Path must start with HKLM/HKCU/HKCR/HKU/HKCC. With 'value', returns just that property; without it, returns the key's values plus its immediate subkeys. Use this for inspecting installed software, autoruns, service config — far better than writing PowerShell from scratch.
